...
def signIn(connection):
    ...
    if username=='' or password=='':
        ...
        exit()
    data = "{'u':'"+str(username)+"', 'p':'"+str(password)+"'}"
    connection.sendall(sendData(data,101))       #向服务器发送消息，注意数字10000。尝试修改实验，对接login.py中的方法名XXXX_101
...
def sendMessage(connection, returnLogin):
    ...
    while 1:
        ...
        try:
            #print returnLogin
            data = "["+str(returnLogin['user_id'])+",'"+str(returnLogin['user_name'])+"','"+str(returnLogin['userkey'])+"', '"+data+"']"
        except Exception, e:
            print u"登陆签名错误，用户退出"
            return
        ...
        connection.sendall(sendData(data,10001))                        #向服务器发送消息，注意10001，对接server.py
        print line
